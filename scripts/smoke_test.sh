...
ck "uXRCE Agent"        "~/Micro-XRCE-DDS-Agent/build/MicroXRCEAgent --help"
ck "磁盘>10G"           "bash -c '[ \$(df --output=avail -B1 ~ | tail -1) -gt 10737418240 ]'"
echo "[smoke] MCP 握手"
H='{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"s","version":"0"}}}'
ck "mcp-rosbags"        "printf '%s\n' '$H' | MCP_ROSBAG_DIR=/tmp PYTHONPATH=$HOME/ai-skills/mcp-rosbags/src timeout 8 $HOME/ai-skills/mcp-rosbags/venv/bin/python $HOME/ai-skills/mcp-rosbags/src/server.py 2>/dev/null | grep -q serverInfo"
ck "ros-mcp"            "bash -c \"source /opt/ros/humble/setup.bash; printf '%s\n' '$H' | ROS_DOMAIN_ID=77 timeout 8 $HOME/ai-skills/ros-mcp/venv/bin/python $HOME/ai-skills/ros-mcp/ros-general.py 2>/dev/null | grep -q serverInfo\""
echo "[smoke] 栈起停+数据流"
cd "$(dirname "$0")/.."
